...
class Dualpath_block(torch.nn.Module):
    ...
    def rnn_forward(self, x):
        ...
        out = out.permute(2, 0, 1)  # L, B, 2M
        out=self.rnn(out)[0]  # L, B, M
        out = out.permute(1, 2, 0)  # B, M, L

        out=self.activation(out)
        out=self.linear(out.transpose(-2, -1)) # B, L, M
        out = self.norm_rnn(out.transpose(-2, -1))  # B, M, L

        out = out.view(B, 1, M, T)  # B, 1, M, L
        return out
