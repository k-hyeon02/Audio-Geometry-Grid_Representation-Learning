...
def get_fixed_array(name: str) -> np.ndarray:  # 고정 배열 이름으로 마이크 좌표 배열 조회
    arrays = {  # 지원하는 고정 배열 종류
        ...
        "nao4": NAO_4CH,                  # NAO 4채널
        "nao12": NAO_ROBOT_12CH,          # LOCATA/EARS 12채널
    }
    try:
...
def random_rotation_matrix(rng: np.random.Generator) -> np.ndarray:  # 무작위 3×3 회전 행렬 생성 (균일 분포)
    ...
    q = np.array(  # 균일분포의 쿼터니언 (3D 회전 공간에서 균등하게 샘플링)
        [
...
def sample_dynamic_array(num_channels: int,
    ...
    for _ in range(num_channels):  # 각 마이크마다 위치 결정
        ...
        angle = 2.0 * np.pi * len(positions) / num_channels  # 원 위의 각도 (균등 분배)
    ...
    coords += rng.uniform(RORIGIN_CM[0], RORIGIN_CM[1], size=coords.shape).astype(  # jitter 추가 (±0.5cm)
        np.float32
    ) / 100.0  # cm → m로 변환
